...
from agentcop_sign import scanner
...
def test_scan_zip_passes_on_high_score(tmp_zip: Path):
    ...
    with patch("requests.post", return_value=mock_resp):
        result = scanner.scan_zip(str(tmp_zip), "https://agentcop.live/api/scan")
...
def test_scan_zip_http_error_raises(tmp_zip: Path):
    ...
    with patch("requests.post", return_value=mock_resp):
        with pytest.raises(RuntimeError):
            scanner.scan_zip(str(tmp_zip), "https://agentcop.live/api/scan")
