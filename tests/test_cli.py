...
from agentcop_sign.models import ScanResult, VerifyResult
...
def test_sign_success(runner, fresh_zip):
    with patch("agentcop_sign.scanner.scan_zip", return_value=_mock_scan_pass()), \
         patch("agentcop_sign.client.register", side_effect=Exception("offline")):
        result = runner.invoke(main, ["sign", str(fresh_zip), "--offline"])
    assert result.exit_code == 0
    assert "Signed successfully" in result.output
    assert "License:" in result.output
    assert "verify.agentcop.live" in result.output


def test_sign_scan_failure_exits_1(runner, fresh_zip):
    with patch("agentcop_sign.scanner.scan_zip", return_value=_mock_scan_fail()):
        result = runner.invoke(main, ["sign", str(fresh_zip), "--offline"])
    assert result.exit_code == 1
    assert "Scan failed" in result.output or "Scan failed" in (result.output + (result.exception or ""))
...
def test_revoke_success(runner):
    with patch("agentcop_sign.client.revoke", return_value=True):
...
def test_revoke_failure_exits_1(runner):
    with patch("agentcop_sign.client.revoke", side_effect=Exception("forbidden")):
        result = runner.invoke(main, ["revoke", "AC-1234-ABCD", "--api-key", "mykey"])
    assert result.exit_code == 1
...
def test_badge_enterprise_tier(runner):
    result = runner.invoke(
        main,
        ["badge", "AC-ENT-1234-ABCD", "--tier", "enterprise", "--expires-at", "2027-12-31T00:00:00Z"],
    )
    assert result.exit_code == 0
